...
class gdrts_core_query {
    ...
    protected function prepare_query_parts() {
        $parts = array(
            'select' => $this->args['return'] == 'ids' ? 'i.`item_id`' : 'i.*'.
                        ", CAST(m.`meta_value` AS DECIMAL(10,2)) as rating",
    ...
    protected function parse_order($q) {
        if ($this->args['orderby'] != '' && $this->args['orderby'] != 'none') {
            if ($this->args['orderby'] == 'rand') {
                ...
                if (($orderby == 'up' || $orderby == 'down') && $this->args['method'] != 'thumbs-rating') {
                    $orderby = 'rating';
                }
                ...
                switch ($orderby) {
                    ...
                    case 'latest':
                        ...
                        break;
                    case 'up':
                        $q['order'].= 'up '.$order.', rating';
                        break;
                    case 'down':
                        $q['order'].= 'down '.$order.', rating';
                        break;
                    case 'votes':
                        $q['order'].= 'votes';
                        break;
    ...
    protected function _run() {
        $items = gdrts_db()->run_and_index($this->sql, 'item_id');
